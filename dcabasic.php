...
{ 
 $output = '';
 $query = "SELECT * FROM course_tb ORDER BY dcabasic ASC";
...
<div class="card-panel mainwork">
        <div class="row">
            <h4 align="center">Dominion City Academy (Basic)</h4>
            <hr>
            <div class="col m3 s5">
...
$(document).on('click', '#scoresheet', function(){
 var sheet = "";
   ...
   sheet += '<form method="post" id="insert_form">'
   ...
   sheet += '<input type="submit" id="submit" name="submit" class="btn" value="Insert" />'
   sheet += '</div>'
   sheet += '</form>';
...
$(document).on('click', '#viewscore', function(){
 ...
 $(document).on('blur', '#gradtable', function(){
    var basic = $('.count').attr('id');
    var graddate = $('.date').val();
    var soul = $('.soul').val();
    $.ajax({
    url:"includes/insert.php",
    method:"POST",
    data:{basic:basic,graddate:graddate,soul:soul},
    success:function(data)
    {
     $('#alert').html(data);
    }
